...
/**
 * @param {{ runId: string, createdAt: string, signals: Array<{ id: string, title: string, decisionQuestion: string, summary: string, confidence: "high" | "medium" | "low", sourceDocuments: string[], evidence: EvidenceReference[] }>, decisions: Array<{ signalId: string, status: "approved" | "edited" | "pending", decidedAt: string, rationale?: string }> }} input
 */
export function createEnrichmentAuditMarkdown(input) {
  const decisionsBySignal = new Map(input.decisions.map((decision) => [decision.signalId, decision]));
  const signalSections = input.signals.flatMap((signal, index) => {
    const decision = decisionsBySignal.get(signal.id);
    const decisionStatus = decision ? titleCase(decision.status) : "Pending";
    const evidenceRows = signal.evidence.map((evidence) => [
      evidence.fileName,
      evidence.locator,
      evidence.excerpt,
    ].map(markdownCell).join(" | "));
    ...
    return [
      ...
      "",
      `- **Decision question:** ${signal.decisionQuestion}`,
      `- **Confidence:** ${titleCase(signal.confidence)}`,
      `- **Source documents:** ${signal.sourceDocuments.map(markdownCell).join(", ")}`,
      `- **Decision:** ${decisionStatus}${decision ? ` (${decision.decidedAt})` : ""}`,
  ...
  return [
    ...
    `- **Run:** ${input.runId}`,
    `- **Created:** ${input.createdAt}`,
    `- **Signals assessed:** ${input.signals.length}`,
    "",
    ...signalSections,
  ].join("\n");
}
