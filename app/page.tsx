...
export default function Home() {
  ...
  return <main className="intake-shell">
    ...
    <div className="product-layout">
      <aside className="workflow-rail" aria-label="Five-step workflow"><span className="rail-label">WORKFLOW</span><ol>{workflowSteps.map((step, index) => <li key={step} className={index === 0 ? "active-step" : "upcoming-step"}><span>{String(index + 1).padStart(2, "0")}</span><div><b>{step}</b><small>{index === 0 ? "In progress" : "Upcoming"}</small></div></li>)}</ol><p><i /> Human review is built into every stage.</p></aside>
